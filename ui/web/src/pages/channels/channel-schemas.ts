...
export const configSchema: Record<string, FieldDef[]> = {
  telegram: [
    ...
    // [fork] Custom pairing text — placeholders: {code}, {bot_name}. Empty = i18n default.
    { key: "pairing_dm_text", label: "Custom DM Pairing Text", type: "textarea",
      placeholder: "🔗 Tài khoản chưa được ghép nối.\n\nMã: {code}\n\nChia sẻ mã này với chủ bot ({bot_name}).",
      help: "Shown when an unknown user DMs the bot. Placeholders: {code}, {bot_name}. Leave empty to use the default message." },
    { key: "pairing_group_text", label: "Custom Group Pairing Text", type: "textarea",
      placeholder: "🔗 Nhóm chưa được ghép nối.\n\nMã: {code}\n\nChia sẻ với chủ bot ({bot_name}).",
      help: "Shown when the bot is added to a new group requiring approval. Placeholders: {code}, {bot_name}." },
    { key: "pairing_approved_text", label: "Custom Approved Text", type: "textarea",
      placeholder: "✅ {bot_name} đã được cấp quyền. Gửi tin nhắn để bắt đầu.",
      help: "Shown after an admin approves the pairing request. Placeholder: {bot_name}." },
    { key: "pairing_locale", label: "Pairing Fallback Locale", type: "select",
      options: [{ value: "en", label: "English" }, { value: "vi", label: "Tiếng Việt" }, { value: "zh", label: "中文" }],
      defaultValue: "en",
      help: "Language used for default pairing messages when the custom text fields above are empty." },
  ],
  discord: [
    { key: "dm_policy", label: "DM Policy", type: "select", options: dmPolicyOptions, defaultValue: "pairing" },
    { key: "group_policy", label: "Group Policy", type: "select", options: groupPolicyOptions, defaultValue: "pairing" },
